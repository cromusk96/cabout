...
langFromURL =
  urlParamsKamp.get("lang") !== null ? urlParamsKamp.get("lang") : "";
...
//const accessToken ="pk.eyJ1IjoidHJpbXJkIiwiYSI6ImNtNW1heHJzbjB2c2Uya3NjdWdmZnR4dm0ifQ.cNGh8ldIjEewUCDUvZRkwA";
...
const map = new mapboxgl.Map({
  ...
  //style: "mapbox://styles/joso/clxsnwj9s00qb01pc8rm8c0a6/draft",
...
*/
    ...
    geolocate.on("geolocate", function (e) {
      if (trackActive == false) {
        map.once("idle", function (e) {
          if (prviprikaz == true) {
            $("#map").css("pointer-events", "auto");
            $("#home, #nav-3d, #return, #car, #walk, #audio").removeAttr(
              "style"
            );
            prviprikaz = false;
            setTimeout(function () {
            ...
            $("#voice").remove();
            $("body").append(
              `<div class="banner">${poly.route[0].legs[0].steps[0].instructions}</div>`
            );
            $("body").append(`<audio id="voice" ${
              isAppleDetected ? isAppleMuted : isMuted
            } autoplay>
            <source src="https://www.camp2guest.com/asdf${
              lng == "en" ? "" : "_" + lng
            }/api${lng == "en" ? "" : "_" + lng}/s?text=${
              poly.route[0].legs[0].steps[0].instructions
            }.${lng == "hr" ? "&speaker_id=hr" : ""}" type="audio/wav">
            </audio>`);
          }
        });
        ...
        if (linestring != undefined && odradjeno == true) {
          ...
          var segment =
            poly.route[0].legs[0].steps[tockeIteracija].geometry.coordinates;
          var segmentCisti =
            poly.route[0].legs[0].steps[tockeIteracija].geometry;
          var segmentInstru =
            poly.route[0].legs[0].steps[tockeIteracija].instructions;
          ...
          var segmentSlijedeci =
            poly.route[0].legs[0].steps[tockeIteracija + 1].geometry
              .coordinates;
          ...
          $(".time-distance .distance").html(
            `${formatTimeDistance.metric(length)} |`
          );
          $(".time-distance .time").html(
            `${formatTimeDistance.duration((length / speed) * 3.6)}`
          );
          ...
          if (d2 < d && d1 < d) {
            ...
            <source src="https://www.camp2guest.com/asdf${
              lng == "en" ? "" : "_" + lng
                ...
                tockeLegs[tockeIteracija + 1].instructions
              }.${lng == "hr" ? "&speaker_id=hr" : ""}" type="audio/wav">
            </audio>`);
            ...
            $("body").append(
              ...
              <div class="banner-top">
                <img src="assets/img/arrows/${
                  modifier == undefined && type == "arrive" ? type : modifier
                }.svg"/>
            ...
            if (type == "arrive" && d2 < 10) {
              ...
              $("body").addClass("message");
              $("body").append(`<div class="annotation gps-done"><h3>${
                translations["Travel done"][0][lng]
              }</h3>
            <p>${
              translations["Destination reached"][0][lng]
            } ${formatGPSDistance["metric"](Math.floor(d2))}.</p>
  ...
  directions.on("route", function (e) {
    ...
    if (poly.route.length > 0) {
      ...
      poly.route[0].geometry = polylineCustom.toGeoJSONCustom(
        poly.route[0].geometry,
        5
      );
    ...
    if (poly.route.length == 0) {
      ...
      $("body").addClass("message");
      //alert('testin2')
      $("body").append(`<div class="annotation"><h3>${
        translations["Imporant message"][0][lng]
      }</h3>
  <p>${translations["Routing not available"][0][lng]}
  ${
    $(".profile-switch.active").attr("id") == "car"
      ? translations["Switch walking"][0][lng]
      : translations["Switch driving"][0][lng]
  }</p>
  <div class="annotation-buttons"><div class="confirm">${
    translations["Fine"][0][lng]
  }</div></div></div>`);
